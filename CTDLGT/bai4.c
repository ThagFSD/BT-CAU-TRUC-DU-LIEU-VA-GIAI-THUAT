...
int TimNoiSuy2(int a[], int N, int X, int *ss) {
    int left = 0, right = N - 1;
    *ss = 0; 
    while (left <= right) { 
        (*ss)++; 
        ...
        int pos = right - ((double)(left - right) / (a[right] - a[left])) * (a[left] - X); 
...
int main() {
    ...
    if (KiemTraTang(a, N)) {
        ...
        kq = TimNoiSuy2(a, N, x, &ss); // Sử dụng tìm kiếm nội suy cho mảng giảm dần
        printf("  - Noi suy (mang giam): ");
    } else {
